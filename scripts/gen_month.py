...
PRODUCTS = {
    ...
    "makeup":        ("https://www.wildberries.ru/catalog/1186405726/detail.aspx", "https://www.ozon.ru/product/4569739061",
                      "салфетки для снятия макияжа, 30 штук в упаковке — бережно снимают макияж и очищают кожу лица, удобны в поездке и когда нет доступа к воде"),
    "extract_romashka": ("", "https://www.ozon.ru/product/4946629934",
                      "экстракт для купания новорождённых с РОМАШКОЙ — добавляется в воду для купания малыша, мягкий уход за нежной детской кожей (это средство ДЛЯ ВАННЫ, не лекарство)"),
    "extract_pihta": ("", "https://www.ozon.ru/product/4945955292",
                      "экстракт для купания новорождённых с ПИХТОЙ — добавляется в воду для купания малыша, хвойный уход за детской кожей (это средство ДЛЯ ВАННЫ, не лекарство)"),
}
# товары НЕ про полость рта — им подбираем нейтральные ракурсы (см. ANGLES_UNIVERSAL)
NONORAL = {"makeup", "extract_romashka", "extract_pihta"}
...
# нейтральные ракурсы для товаров НЕ про полость рта (макияж, экстракты для купания)
ANGLES_UNIVERSAL = [
    "в дороге и путешествии, когда нет доступа к воде",
    "быстрый уход в потоке дел, когда мало времени",
    "вечерний ритуал перед сном",
    "забота о себе и о малыше каждый день",
    "будни молодой мамы: сборы и мелочи, которые выручают",
    "что удобно держать в сумке и брать с собой",
    "простые ритуалы ухода, которые легко войдут в привычку",
]

BRIEF = """Ты — текстолог бренда «АС Фарм» (российский производитель средств гигиены и ухода для всей семьи: гигиена полости рта, уход за кожей, купание малышей). Пиши спокойно, по-человечески, только факты. БЕЗ эмодзи в тексте (кроме финальной подписи), без канцелярита, без «важно отметить/в заключение», без списков-штампов, без восклицаний.
...
- Салфетки для снятия макияжа — уход за КОЖЕЙ ЛИЦА (снимают макияж, очищают), НЕ для полости рта.
- Экстракты для купания новорождённых — средство ДЛЯ ВАННЫ (добавляется в воду при купании малыша), бывают с ромашкой и с пихтой; это НЕ лекарство и НЕ для приёма внутрь.
- Пиши строго про ТОВАР ЭТОГО ПОСТА (см. ниже) — не смешивай его с другими линейками.
...
def generate(n, posts, gkey):
    ...
    while made < n and attempts < n * 4:
        ...
        _angles = ANGLES_UNIVERSAL if pkey in NONORAL else ANGLES
        angle = _angles[(len(posts) + attempts) % len(_angles)]
